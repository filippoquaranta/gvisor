	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				...
				t.Fatalf("numBenchmarks want: %d got: %d %+v", tc.numBenchmarks, len(bms), bms)
			}

			for _, bm := range bms {
				if len(bm.Metric) != tc.numMetrics {
					t.Fatalf("numMetrics want: %d got: %d %+v", tc.numMetrics, len(bm.Metric), bm.Metric)
				}

				if len(bm.Condition) != tc.numConditions {
					t.Fatalf("numConditions want: %d got: %d %+v", tc.numConditions, len(bm.Condition), bm.Condition)
				}
			}
		})
	}
