...
func addNewLinkMessage(ms *netlink.MessageSet, idx int32, i inet.Interface) {
	...
	mtu := primitive.Uint32(i.MTU)
	m.PutAttr(linux.IFLA_MTU, &mtu)
	...
	macP := primitive.ByteSlice(mac)
	brdP := primitive.ByteSlice(brd)
	m.PutAttr(linux.IFLA_ADDRESS, &macP)
	m.PutAttr(linux.IFLA_BROADCAST, &brdP)
